...
if (isset($_GET['code'])) {
  ...
  if (!isset($token['error'])) {
    ...
    $result = $link->query("SELECT * FROM account where email='{$userinfo['email']}'");
    if ($result->rowCount() == 0) 
	{
      $link->query("insert into account values(uuid(),'{$userinfo['full_name']}','{$userinfo['full_name']}','{$userinfo['token']}','{$userinfo['email']}',Null,'$today','$today','{$userinfo['picture']}',Null,Null,5)");
    }
    //自己寫的Function
